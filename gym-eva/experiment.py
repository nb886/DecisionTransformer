...
from d_mindformer.models.d_mindformer import DecisionMindformer
...
def experiment(
    ...
    def get_batch(batch_size=256,
                  max_len=K):  # get_batch函数通常用于从环境（Environment）或数据存储器（如 replay buffer）中获取一批（batch）状态、动作、奖励等。这些构成了一个经验（Experience）的序列，可以用于训练神经网络或策略
        ...
        for i in range(batch_size):
            ...
            s.append(traj['observations'][si:si + max_len].reshape(1, -1,
                                                                   state_dim))  # 取index在si到si+20之间的state  # 20就是context，也就是NLP输出词语时考虑的语境范围
            ...
            rtg.append(discount_cumsum(traj['rewards'][si:], gamma=1.)[:s[-1].shape[1] + 1].reshape(1, -1,
                                                                                                    1))  # returntogo 这个点到最后一个点(当前episode)的reward总和，这里的最后一个点不是si+20
    ...
    if log_to_wandb:
        ...
        wandb.login(key='6a4a014d20101c4d1dd5e3ae5566c72755be2221')
        wandb.init(
            name=exp_prefix,
            group=group_name,
    ...
    for iter in range(variant['max_iters']):
        outputs = trainer.train_iteration(num_steps=variant['num_steps_per_iter'], iter_num=iter + 1,
                                          print_logs=True)  # 将SequenceTrainer进行train_iteration,减小loss
        if log_to_wandb:
            wandb.log(outputs)
...
if __name__ == '__main__':
    ...
    experiment('gym-experiment-dt', variant=vars(args))
