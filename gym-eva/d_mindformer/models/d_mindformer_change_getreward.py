...
class DecisionMindformer(TrajectoryModel):
    ...
    def __init__(
        ...
        # self.mindformer = mindformers.models.gpt2.gpt2.GPT2Model(config)
    ...
    def forward(self, states, actions, rewards, returns_to_go, timesteps, attention_mask=None):
        ...
        # print('returns_embeddings=',returns_embeddings.shape)
        # print('action_embeddings=',state_embeddings.shape)

        # this makes the sequence look like (R_1, s_1, a_1, R_2, s_2, a_2, ...)
        # which works nice in an autoregressive sense since states predict actions
        stacked_inputs = (torch.stack(
            (returns_embeddings, state_embeddings, action_embeddings, rewards_embeddings), dim=1
        ))
        ...
        reward_preds = self.predict_reward(x[:,1])  # predict next action given state    [:,2]初测效果不好
# 5- 1211  4-2211   3-2212
    ...
    def get_action(self, states, actions, rewards, returns_to_go, timesteps, **kwargs):
        ...
        reward_preds_a = self.get_reward(states,actions, rewards, returns_to_go, timesteps, attention_mask=attention_mask)


        _, action_preds_b, reward_preds_a, reward_preds = self.forward(
            states, actions, reward_preds_a, returns_to_go, timesteps, attention_mask=attention_mask)
